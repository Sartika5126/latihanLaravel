<x-app-layout>
    <div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow mt-8">
        <h2 class="text-xl font-semibold mb-4">Step 4 - Alamat Domisili $ Referensi Pendaftaran</h2>
        ...
        <form method="POST" action="{{ route('ekyc.step4.store') }}" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <select id="provinsi" name="provinsi" class="mt-1 block w-full border-gray-300 rounded-md">
                    <option value="">-- Pilih Provinsi --</option>
                    @foreach($provinsiList as $prov)
                        <option value="{{ $prov }}" {{ old('provinsi', $data->provinsi) == $prov ? 'selected' : '' }}>{{ $prov }}
                        </option>
                    @endforeach
                </select>      
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="kota" name="kota" class="mt-1 block w-full border-gray-300 rounded-md">  
                     <option value="">-- Pilih Kota --</option>
                    @foreach($kotaList as $kota)
                        <option value="{{ $kota }}" {{ old('kota', $data->kota) == $prov ? 'selected' : '' }}>{{ $kota }}
                        </option>
                    @endforeach
                    </select>      
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="kecamatan" name="kecamatan" class="mt-1 block w-full border-gray-300 rounded-md">  
                     <option value="">-- Pilih Kecamatan --</option>
                     @foreach($kecamatanList as $kec)
                        <option value="{{ $kec }}" {{ old('kecamatan', $data->kecamatan) == $kec ? 'selected' : '' }}>{{ $kec }}
                        </option>
                    @endforeach
                </select>      
            </div>
            ...
            <div class="mb-4">
                ...
                <select id="referensi_sumber" name="referensi_sumber" class="mt-1 block w-full border-gray-300 rounded-md">  
        ...
        document.getElementById('kecamatan').addEventListener('change', function() {
            ...
            // Cari data alamat yang cocok untuk mengisi kode pos
            const selected = alamatData.find(item => item.kecamatan === kec);
            
            // Isi nilai input kode pos
